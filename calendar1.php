...
//Postされた全てのスケジュールをqueryに入れれるように直してSessionに登録
if(isset($_POST["schID"])){
	$_SESSION["schID"]=$_POST["schID"];
}
//スケジュールのタスクを出力処理
if (!$_SESSION["schID"]=="") {
	$schIds=explode(",",$_SESSION["schID"]);
	$schIdbefore="";
	$schId="";
	$schIdnext="";
	$date = $this_year."-".$this_month."-%";
	for ($i=0;$i<(count($schIds)-1);$i++) {
		if ($i==(count($schIds)-2)) {
			$schIdbefore .= "( s_id='".$schIds[$i]."' and t_date between '$date_before_m-20' and '$date_before_m-31' )";
			$schId .="( s_id='".$schIds[$i]."' and  t_date LIKE '$date')";
			$schIdnext .="( s_id='".$schIds[$i]."' and t_date between '$date_next_m-01' and '$date_next_m-15' )";
		}else{
			$schIdbefore .= "( s_id='".$schIds[$i]."' and t_date between '$date_before_m-20' and '$date_before_m-31' ) or ";
			$schId .=" ( s_id='".$schIds[$i]."' and  t_date LIKE '$date') or  ";
			$schIdnext .="( s_id='".$schIds[$i]."' and t_date between '$date_next_m-01' and '$date_next_m-15' ) or ";
		}
	}
	include('config/db_ini.php');
	//前月のタスク検索
	$sql1="select * from schedule_task_table where $schIdbefore";
	...
	//今月のタスク検索
	$sql="select * from schedule_task_table where $schId ";
	...
	//来月のタスク検索
	$sql2="select * from schedule_task_table where $schIdnext ";
...
for($i = 1; $i <= 31; $i++){
	if (checkdate($this_month,$i,$this_year) == true) {
		...
		if ($i == 1) {
			...
			while ($y < $youbi) {
				...
				if(checkdate($before_m,$i,$yyyyb)== true){
					...
					if ($u==1) {
						$msg .= "<td>";
						$msg .= "<div class='settasksize notmonth'>";
						...
						for ($m=0;$m<$db_cnt1; $m++) {
							if(isset($taskdate_before_month[$m][$before_m_l_d-$v])) {
								...
								$msg .= "<div  onclick=showtask('$taskdate_before','$taskid_before') class='taskbar'>".$taskdate_before_month[$m][$before_m_l_d-$v]."</div>";
								$msg.="<div class='outer cal11'>";
								$msg .="<div class='inner'>";
								$msg .= "<a class='call11close' onclick=deletetask('$taskdate_before','$taskid_before')>delete</a>";
								$msg .= "</div>";
								$msg .= "</div>";
								$msg .= "</div>";
							}
		...
		if ($i==$this_day && $this_month == date("m") && $this_year == date("Y")) {
			$msg .= "<td>";
			$msg .= "<div  class='settasksize today'  >";
			...
			for ($x=0;$x<=$db_cnt; $x++) {
				if(isset($taskdate[$x][$i])) {
					...
					// code...
					$msg .= "<div class='progressbar'>";
					$msg .= "<div onclick=showtask('$taskname','$taskid1') class='taskbar'>".$taskdate[$x][$i]."</div>";
					$msg.="<div class='outer cal11'>";
					$msg .="<div class='inner'>";
					$msg .= "<a class='call11close' onclick=deletetask('$taskname','$taskid1')>delete</a>";
					$msg .= "</div>";
					$msg .= "</div>";
					$msg .= "</div>";
				}
	...
	if ($x==6) {
		...
		for ($j=0;$j<$db_cnt2; $j++) {
			if(isset($taskdate_next_month[$j][$d])) {
				...
				$msg .= "<div class='progressbar'>";
				$msg .= "<div onclick=showtask('$task_next','$taskid_next') class='taskbar'>".$taskdate_next_month[$j][$d]."</div>";
				$msg.="<div class='outer cal11'>";
				$msg .="<div class='inner'>";
				$msg .= "<a class='call11close' onclick=deletetask('$task_next','$taskid_next')>delete</a>";
				$msg .= "</div>";
				$msg .= "</div>";
				$msg .= "</div>";
			}
...
<link rel="stylesheet" href="css/calendar.css">
	...
	<div id="taskdisplay"  style="display:none;">
		<div class="outer cal1">
			<div class="inner">
				<a class="cal1close" onclick="taskshowclose()" type="button">close</a>
			</div>
		</div>
	...
	<h1 class="center"><?php print $this_year ?>年<?php print $this_month ?>月 </h1>
		<!-- カレンダー表示 -->
		<?php print $msg?>
